...
<head>
	...
	$username = "";
	$password = "";
	$koneksi = mysqli_connect("localhost","root","","Dabes");

	if (isset($_POST['submit'])) {
		$username = $_POST['username'];
		$password = $_POST["password"];
		if ($username && $password) {
			$sql1 = mysqli_query($koneksi, "select * from from_login where username ='$username'and password = '$password'");
			if ($sql1->num_rows>0) {
				$this->load->view('pertama');
				exit();
			}else {
